...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [systemStatus, analytics, alerts, cacheStats] = await Promise.allSettled([
      // System status
      Promise.resolve({
        uptime: process.uptime(),
        memory: {
    ...
    const dashboard = {
      timestamp: new Date().toISOString(),
      system:
        systemStatus.status === 'fulfilled' ? systemStatus.value : { error: 'Failed to load' },
      ...
      summary: {
        overallHealth:
          ...
          systemStatus.value.services.redis === 'healthy'
            ? 'healthy'
            : 'degraded',
        criticalAlerts:
          alerts.status === 'fulfilled'
            ...
            : 0,
        requestsLastHour:
          analytics.status === 'fulfilled' ? analytics.value.summary.recentRequests : 0,
      },
    };
